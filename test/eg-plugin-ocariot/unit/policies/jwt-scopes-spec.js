const policy = require('../../../../eg-plugin-ocariot/policies/authorization/jwt-scopes-policy')
const assert = require('chai').assert
const sinon = require('sinon')

describe('Policy: ocariot-jwtScopes-policy', () => {
    describe('Integrity', () => {
        it('Field "name" is type valid', () => {
            assert.typeOf(policy.name, 'string', '"name" is not string type')
            assert.equal(policy.name, 'ocariot-jwtScopes-policy', 'Policy name other than expected')
        })
        ...
        it('Field "schema.$id" is type valid', () => {
            ...
            assert.equal(policy.schema.$id, 'http://express-gateway.io/schemas/policies/ocariot-jwtScopes-policy.json', '"schema.$id" different than expected')
        })
